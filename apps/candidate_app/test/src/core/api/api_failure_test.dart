...
import 'package:auto_skola_365_candidate_app/src/core/api/api_client.dart';
import 'package:auto_skola_365_candidate_app/src/core/api/failure.dart';
import 'package:auto_skola_365_candidate_app/src/features/portal/data/datasources/portal_remote_data_source.dart';
import 'package:auto_skola_365_candidate_app/src/features/portal/data/repositories/portal_repository_impl.dart';
...
void main() {
  ...
  for (final status in [400, 401, 403, 404, 409, 500]) {
    ...
    test('repository returns Left with the original $status details', () async {
      ...
      final result = await PortalRepositoryImpl(
        PortalRemoteDataSource(api),
      ).load(schoolId: 'school', accessToken: 'token');
